...
Se ha implementado un nuevo subsistema de actualización en Python (`src/update_system.py`) que reemplaza al antiguo script de Node.js.
*   **Funcionalidad**: Al pulsar "Actualizar Datos" en el dashboard, el sistema:
    1.  Ejecuta un **Scraper de Puppeteer** (`src/scraper_winamax.js`) que extrae las cuotas REALES de Winamax `window.PRELOADED_STATE` (Live Odds).
    2.  Descarga los últimos resultados de LaLiga 24/25.
    3.  Recalcula todas las métricas (Elo, xG, etc.) al instante y actualiza el Excel.
*   **Ventaja**: Permite mantener el dashboard vivo con datos verificados y cuotas reales de mercado.
